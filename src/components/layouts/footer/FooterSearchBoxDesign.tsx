    ...
    return (
        <div>
            <div className="lg:max-w-5xl md:max-w-3xl h-48  md:h-40 flex flex-col md:flex-row items-center justify-center md:gap-4 gap-6 bg-[#212139] w-full md:rounded-md rounded-lg absolute -top-20 left-1/2 right-1/2 -translate-x-1/2 md:px-20 px-6">
                {/* Toggle Buttons */}
                <div className="md:w-1/4 w-full font-semibold text-textCol text-center flex gap-1 border p-1 rounded-md bg-textCol">
                    <button
                        onClick={() => setSelectedOption('Auto')}
                        className={`flex-1 px-0 py-2 rounded-md ${selectedOption === 'Auto' ? 'gradient-color text-textCol' : 'bg-textCol text-secondary'
                            }`}
                    >
                        Auto
                    </button>
